  ...
  try {
    ...
    switch (command) {
      case 'backup': case 'copia': {
        ...
        const creds = await fs.readFile('./Sessions/Principal/creds.json');
      ...
      case 'jadibot': case 'serbot': {
        ...
        const isEnable = global.db.data.settings[conn.user.jid][type] || false;
        ...
        if (enable || disable) {
          ...
          global.db.data.settings[conn.user.jid][type] = enable;
