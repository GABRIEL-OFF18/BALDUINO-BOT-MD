...
let love = `*${text1}* tu oportunidad de enamorarte de *${text2}* es de ${Math.floor(Math.random() * 100)}%`
...
let texto = cantidad === 1 ? `*La mejor pareja del grupo*\n\n` : `*Las ${cantidad} mejores parejas del grupo*\n\n`
parejas.forEach((p, i) => {
...
switch (command) {
case 'gay':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* Gay\nEso es bajo, ¡Tú eres Joto, no Gay!` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* Gay\n¡Incluso más gay de lo que pensábamos!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* Gay\nLo tuyo, lo tuyo es que eres Gay.`
break
case 'lesbiana':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nQuizás necesites más películas románticas en tu vida.` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\n¡Eso es un amor extremo por las Chicas!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nMantén el amor floreciendo!`
break
case 'pajero': case 'pajera':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nTal vez necesites más hobbies!` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nEso es una resistencia admirable!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nMantén el buen trabajo (en solitario).`
break
case 'puto': case 'puta':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\n¡Más suerte en tu próxima conquista!` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\n¡Estás en llamas!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nMantén ese encanto ardiente!`
break
case 'manco': case 'manca':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\n¡No eres el único en ese club!` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\n¡Tienes un talento muy especial!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nMantén esa actitud valiente!`
break
case 'rata':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nNada de malo en disfrutar del queso!` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nUn auténtico ratón de lujo!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nCome queso con responsabilidad!`
break
case 'prostituto': case 'prostituta':
description = percentages < 50 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nEl mercado está en auge!` : percentages > 100 ? `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nUn/a verdadero/a profesional!` : `Los cálculos han arrojado que ${userId} es *${percentages}%* ${command}\nSiempre es hora de negocios!`
break
}
